...
class PersonController {
  ...
  def show() {
    def result=[:]
    result.person = Person.get(params.id)
    result.works = AuthorName.executeQuery("select a from AuthorName as a where a.matchedPerson = ?",[result.person])
    result
  }
